...
class CustomDriver(DeepDriveMDDriver):
    ...
    def __init__(self, rc=None, system=None):
        ...
        self.base_training_data_path = expandvars(
            "$WEST_SIM_ROOT/common_files/train.npy"
        )
    ...
    def kmeans_cluster_segments(
        self, df: pd.DataFrame, cluster_z: np.ndarray
    ) -> pd.DataFrame:
        # Perform the K-means clustering
        cluster_labels = KMeans(n_clusters=self.kmeans_clusters).fit(cluster_z).labels_
        # Save the cluster context data
        self.save_cluster_context(
            cluster_labels,
            self.cluster_dcoords,
            self.cluster_pcoords,
        ...
        if self.niter % 10 == 0:
            self.plot_latent_space(
                cluster_z, cluster_labels, self.cluster_pcoords, self.cluster_weights
            )
    ...
    def gmm_cluster_segments(
        ...
        df["ls_cluster"] = seg_labels
        # Assign the outliers
        df = self.assign_density_outliers(
            df, cluster_z, seg_labels, cluster_z[: self.nsegs]
        )
        return df
    ...
    def sort_df_for_density(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.target_point is not None:
            by_distance = df.sort_values(
                ["outlier", "distance"], ascending=[False, True]
            )
    ...
    def assign_density_outliers(
        self, df: pd.DataFrame, cluster_z: np.ndarray, cluster_labels: np.ndarray
    ) -> np.ndarray:
        """
        ...
        if -1 in df["ls_cluster"].values:
            ...
            for ind in df[df["outlier"]].inds.values:
                ...
                dist = np.array(
                    [cosine_distance(seg_z[ind], z) for z in inlier_embedding_history]
                )
    ...
    def split_up_to(self, cluster_df: pd.DataFrame, num_resamples: int) -> None:
        ...
        cluster_df = self.remove_underweight_segs(cluster_df).sort_values(
            ["outlier", "pcoord"], ascending=[False, True]
        )
    ...
    def save_cluster_context(
        self,
        cluster_labels: np.ndarray,
        cluster_dcoords: np.ndarray,
        cluster_pcoords: np.ndarray,
        cluster_weights: np.ndarray,
    ) -> None:
        """
        ...
        for label in set(cluster_labels):
            ...
            if label != -1 and len(indices) > self.max_save_per_cluster:
                indices = self.rng.choice(
                    indices, self.max_save_per_cluster, replace=False
                )
        ...
        np.save(
            self.datasets_path / f"cluster-context-dcoords-{self.niter}.npy",
            dcoords_to_save,
        )
        np.save(
            self.datasets_path / f"cluster-context-pcoords-{self.niter}.npy",
            pcoords_to_save,
        )
        np.save(
            self.datasets_path / f"cluster-context-weights-{self.niter}.npy",
            weights_to_save,
        )
    ...
    def load_cluster_context(
        ...
        if self.niter > 1:
            ...
            past_dcoords = np.load(
                self.datasets_path / f"cluster-context-dcoords-{self.niter - 1}.npy"
            )
            past_pcoords = np.load(
                self.datasets_path / f"cluster-context-pcoords-{self.niter - 1}.npy"
            )
            past_weights = np.load(
                self.datasets_path / f"cluster-context-weights-{self.niter - 1}.npy"
            )
    ...
    def run(
        ...
        if self.ml_mode != "ablation":
            ...
            self.machine_learning_method = MachineLearningMethod(
                self.train_path,
                self.base_training_data_path,
                self.static_chk_path,
                self.contact_cutoff,
            )
            ...
            all_dcoords = self.get_prev_dcoords_training(
                next_segments, dcoords, self.lag_iterations
            )
        ...
        if self.knani:
            if isinstance(self.n_clusters, tuple):
                all_labels, output_n_clusters = self.knani_scan_cluster_segments(
                    embedding_history, pcoord_history
                )
                print(f"scanned {self.n_clusters} and used {output_n_clusters}")
            else:
                all_labels = self.knani_cluster_segments(
                    embedding_history, pcoord_history
                )
