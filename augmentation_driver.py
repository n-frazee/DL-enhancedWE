import westpa
import pickle
from synd.westpa.propagator import get_segment_parent_index, get_segment_index
...
class SynDAugmentationDriver:
    ...
    def __init__(self, sim_manager, plugin_config):
        ...
        coord_map_path = plugin_config.get('dmatrix_map')
        with open(coord_map_path, 'rb') as infile:
            self.dmatrix_map = pickle.load(infile)
    ...
    def augment_coordinates(self):
        ...
        feature_shape2 = self.dmatrix_map[0].shape
        ...
        self.data_manager.we_h5file.require_dataset(
            f"{iter_group.name}/auxdata/dmatrix",
            shape=(n_walkers, 2, *feature_shape2),
            dtype=self.dmatrix_map[0].dtype
        )

        self.data_manager.flush_backing()

        auxcoord_dataset = self.data_manager.we_h5file[f"{iter_group.name}/auxdata/coord"]
        auxcoord2_dataset = self.data_manager.we_h5file[f"{iter_group.name}/auxdata/dmatrix"]
        ...
        for i, segment in enumerate(segments):
            ...
            auxcoord2_dataset[segment.seg_id, 0] = self.dmatrix_map[parent_state_index]
            auxcoord2_dataset[segment.seg_id, 1] = self.dmatrix_map[segment_state_index]
